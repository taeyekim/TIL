...
def get_deposit_products():
    ...
    variable2 = requests.get(url).json()["result"]
    base_list = requests.get(url).json()["result"]["baseList"]
    option_list = requests.get(url).json()["result"]["optionList"]
    ...
    for product in base_list:
        ...
        for option in option_list:
            if option['fin_prdt_cd'] in product['fin_prdt_cd']:
                new_option = {}
